...
#include "DDC17_functions.h"

// DDC functions global variables
// Sinus table
#define TABLESZ		48000
_Complex float tableA[TABLESZ];
_Complex float sentcosDDC[1024*1024];
// Filter
int filterlength=0;
...
int IQ_demodulator_carrier_f0(_Complex float *phasor_f0, _Complex float *in, _Complex float *out, int length){
	...
	for(i=0; i<length; i++){
		...
		__imag__ *(out+i)=(__imag__*(phasor_f0+i)) * (__real__ *(in+i));
	}
	return length;
}
...
int IQ_DEMOD(_Complex float *ccinput, int datalength, _Complex float *ccoutput, MODparams_t oParam){
	...
	gen_toneCOMPLEX(sentcosDDC, tableA, TABLESZ, datalength, oParam.f0_freqHz, oParam.samplingfreqHz);	
	// 2) Multiply CPLX vectors
	datalength=IQ_demodulator_carrier_f0(sentcosDDC, ccinput, aux, datalength);
	// Filter Low Pass
	outlength = stream_conv_CPLX_DDC(aux, datalength, COEFFs, filterlength, ccoutput, oParam);
