...
//Module includes
//#include "DDC17_interfaces.h"
#include "DDC17_params.h"
#include "DDC17_functions.h"
#include "DDC17.h"

/* MODULE DEFINED PARAMETERS.*/
char mname[STR_LEN]="DDC17";
extern MODparams_t oParam;
long int Tslot=-1;			/* Time slot number */
...
//		printf("DDC-NORMAL: Tslot=%d, rcv_samples=%d\n", Tslot, rcv_samples);
		if(rcv_samples==0)return(1);
		snd_samples=IQ_DEMOD(input0, rcv_samples, output0, oParam);
	}
	if(oParam.opMODE==BYPASS){
		rcv_samples=get_input_samples(0);
		printf("DDC-BYPASS: Tslot=%d, rcv_samples=%d\n", Tslot, rcv_samples);
		if(rcv_samples==0)return(1);
		bypass(input0, rcv_samples, output0);
