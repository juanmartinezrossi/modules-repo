...
//	SComplexplot complexplot_ctrl;
	...
	_Complex float *tmp_cf;

	_Complex float tmp_cf2[16][1024*50];


	double *plot_buff_r;
	double *plot_buff_c;
	_Complex float *plot_buff_cf;
	_Complex float *plot_buff_cf0;
	_Complex float *plot_buff_cf1;
...
*/
		...
		for (i=0;i<nof_output_itf;i++) {
			printf("OUTPUTS_______________________\n");
			...
			if(strcmp(OUT_TYPE, "COMPLEXFLOAT") == 0){
				printf("IS COMPLEX FLOAT PLOT\n");
				printf("samplingfreqHz=%3.3f, block_length=%d, runtimes=%d\n", samplingfreqHz, block_length, run_times);
				printf("plot_modeOUT=%s, datalen=%d, output_max_samples=%d, output_sample_sz=%d\n",
						plot_modeOUT, output_lengths[i], output_max_samples, output_sample_sz);

				//plot_buff_cf = malloc(sizeof(_Complex float*)*output_lengths[i]);
				plot_buff_cf = (_Complex float*) &output_data[i*output_max_samples];
				printf("tmp_cf=%lu\n", tmp_cf);

	/*			for (j=0;j<output_lengths[i];j++) {
					plot_buff_cf[j] = tmp_cf[j];
				}
	*/

				if(strcmp(plot_modeOUT, "DEFAULT") != 0){
					plotcomplex(plot_modeOUT, plot_buff_cf, output_lengths[i], plot, tmp, &complexplot_ctrl, samplingfreqHz, run_times);
				}else {
					printf("oooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo\n");
					printf("WARNING!!!: Plot default configuration defined at 'graphs_mode.h' setup.                     o\n");
					printf("Please, modify according your needs.                                                         o\n");
				...
				complexplot(plot_buff_cf, output_lengths[i], plot, tmp, &complexplot_ctrl);
			}else{
				if(strcmp(OUT_TYPE, "FLOAT") == 0){
					printf("test_main.c: OUT_TYPE != COMPLEX_FLOAT\n");
				}
...
int param_setup(char *name, char *value, int nofcaptparams){
	...
	//Check if captured params were defined in CHANNEL_ANALYZER_params.h
