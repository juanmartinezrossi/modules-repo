...
{
	int i, data_length=0, k;
	char aux_c;
	int aux_i;
	float aux_f, to, phase;
	...
	freq=50.0*(float)Tslot;
	data_length=block_length*run_times;
	printf("test_generate: Tslot=%d, freq=%3.5f, datalength=%d, phase=%3.5f\n", Tslot, freq, data_length, phase);
	samplingfreqHz=48000.0;
	if(strcmp(IN_TYPE, "COMPLEXFLOAT")==0){
		...
		for (i=0;i<data_length;i++) {
			...
			//__real__ aux_cf=cos(2*3.1415*((double)(i))*freq/samplingfreqHz-(2*3.1415*to*freq));
			__real__ aux_cf=(float)cos(PIx2*((double)(i))*(double)(freq/samplingfreqHz)-(double)phase);
			__imag__ aux_cf=0.0;
