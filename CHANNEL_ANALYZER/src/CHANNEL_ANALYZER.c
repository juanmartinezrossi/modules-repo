...
*/
	...
	printf("O      block_length=%d, sampligfreqHz=%3.3f\n", block_length, samplingfreqHz);
...
int work(input_t *inp, output_t *out) {
	...
	input_t *input0;
	output_t *output0, *output1, *output2, *output3;
	input0=inp;
	output0=out;
	output1=out(out,1);
	output2=out(out,2);
	...
	set_output_samples(2,  0);
	set_output_samples(3,  0);
		...
		if(fase==1){
			...
			if(Tslot <= (scaningTslots+5)*NOFTS){
				...
				if(LTslot == NOFTS-1){
					...
					// Compute phase
					if(modephase == 0)CHphase = get_CH_phase_radB(Csentcos, myin, block_length, aux);
					if(modephase == 1)CHphase = get_CH_phase_radA(Csentcos, myin, block_length, aux);
					if(LTslot==NOFTS-1)saveFLAG=1;
					for(i=0; i<block_length; i++){
						*(output2+i)=*(Csentcos+i);
						*(output3+i)=*(myin+i);
						set_output_samples(2,  block_length);
						set_output_samples(3,  block_length);
					}
				}
				// SAVE GAIN AND PHASE
				if(saveFLAG == 1){ 
