...
void ConfigInterfaces() {
	...
	if (param_init(parameters,nof_parameters) == -1) {
		printf("Error configuring data_source_parameters.h\n");
		return;
	}
	...
	for (i = 0; i < param_nof() && i<nof_parameters; i++) {
		...
		/** get param */
		if (!GetParam(param->name, param_get_addr_i(i),param_get_aloe_type_i(i),param->size)) {
			printf("Caution parameter %s not initiated\n", param->name);
		}
